...
interface IntroDividerProps {
  donor: Inputs; // Adjust the prop type to accept a single donor or null
}

export default function BecomeDonor({ donor }: IntroDividerProps) {
  ...
  const {
    ...
    defaultValues: {
      fullName: donor[0].donorname, // Set default values for each field
      DOB: donor[0].dateofbirth ? new Date(donor[0].dateofbirth) : "date",
      bloodGroup: donor[0].bloodgroup,
      location: donor[0].location,
      disease: donor[0].disease,
      contactNumber: donor[0].contact,
    },
    mode: "onChange",
  });
